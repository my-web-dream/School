             		...
             		<?php else: ?>
						...
						<div class='page'>
							<?php if($page > 1): ?><!-- 首页不显示该信息 -->
								<a href="<?php echo U('Class/student_attend',array('page'=>1,'device_id'=>$device_id,'date'=>$date));?>">首页</a>
								<a href="<?php echo U('Class/student_attend',array('page'=>$page-1,'device_id'=>$device_id,'date'=>$date));?>">上一页</a>
							<?php else: ?>
								<span class='disable'>首页</span>
								<span class='disable'>上一页</span><?php endif; ?>
							<?php if($page < $total_pages): ?><!-- 尾页不显示该信息 -->
								<a href="<?php echo U('Class/student_attend',array('page'=>$page+1,'device_id'=>$device_id,'date'=>$date));?>">下一页</a>
								<a href="<?php echo U('Class/student_attend',array('page'=>$total_pages,'device_id'=>$device_id,'date'=>$date));?>">尾页</a>
							<?php else: ?>
								<span class='disable'>下一页</span>
								<span class='disable'>尾页</span><?php endif; ?>
							共 <?php echo ($total_pages); ?> 页
							当前为第 <?php echo ($page); ?> 页
							<form action="<?php echo U('Class/student_attend',array('device_id'=>$device_id,'date'=>$date));?>" method="get">
