		...
		<div class="n-menu">
			<ul class="n-parmenu">
				<li class="n-firstlevel">
					...
					<ul class="n-submenu">
						<li><a href="<?php echo U('Device/device_add');?>">添加设备</a></li>
						<li><a href="<?php echo U('Device/device_info');?>">设备基本信息</a></li>
					</ul>
				</li>
				<li class="n-firstlevel">
					测试专区
...
$(function(){
	...
	$("#device_detail_info .d-comfire").click(function(){
		...
		$.post("<?php echo U('Device/device_info_update');?>",{id:id,collectRate:collectRate,uploadRate:uploadRate,powerOnDateTime:powerOnDateTime,powerOffDateTime:powerOffDateTime,SOS:SOS,Friend:Friend},function(data,status){
			if(data = 'success'){
				layer.msg('恭喜您，修改配置信息成功！',{time:1500},function(){
	        		window.location.href = "<?php echo U('Device/device_info');?>";  
	        	});
			}else{
				layer.msg("修改失败，请重试");
			}
...
//设备配置信息回填
function editor(id){
	$.post("<?php echo U('Device/device_info_only');?>",{id:id},function(data,status){
		if(data){
...
function cancel(id){	//传入staff_id号
	layer.open({
		  ...
		  yes: function(index,layero){
			$.post("<?php echo U('Device/sch_admin_cancel');?>",{id:id},function(data,status){
			...
			<seciton class="m-content">
				...
				<div class='page'>
					<?php if($page > 1): ?><!-- 首页不显示该信息 -->
						<a href="<?php echo U('Device/device_info',array('page'=>1));?>">首页</a>
						<a href="<?php echo U('Device/device_info',array('page'=>$page-1));?>">上一页</a>
					<?php else: ?>
						<span class='disable'>首页</span>
						<span class='disable'>上一页</span><?php endif; ?>
					<?php if($page < $total_pages): ?><!-- 尾页不显示该信息 -->
						<a href="<?php echo U('Device/device_info',array('page'=>$page+1));?>">下一页</a>
						<a href="<?php echo U('Device/device_info',array('page'=>$total_pages));?>">尾页</a>
					<?php else: ?>
						<span class='disable'>下一页</span>
						<span class='disable'>尾页</span><?php endif; ?>
					共 <?php echo ($total_pages); ?> 页
					当前为第 <?php echo ($page); ?> 页
					<form action="<?php echo U('Device/device_info');?>" method="get">
