...
function detail(device_id,name){
	$.post("<?php echo U('Class/bind_detail');?>",{device_id:device_id},function(data,status){
		if(data){	//返回数据成功
					...
					<table>
						...
						<tbody>
							<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<a href="javascript:void(0)" data-reveal-id="de_bind" data-animation="fade" class="add-member" onclick="detail('<?php echo ($va["device_id"]); ?>','<?php echo ($va["name"]); ?>')">查看详情</a>
									&nbsp;&nbsp;&nbsp;
									<a href="#" class="add-member" onclick="cancel()">解除绑定</a>
								</td>
